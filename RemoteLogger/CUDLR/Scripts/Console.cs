...
namespace CUDLR
{
    ...
    public class Console
    {
        ...
        public static void Update()
        {
            while (Instance.m_commandQueue.Count > 0)
            {
                ...
                cmd.command.m_callback(cmd.context, cmd.args);
            }
        }

        /* Queue a command to be executed on update on the main thread */
        public static void Queue(CommandAttribute command, RequestContext context, string[] args)
        {
            QueuedCommand queuedCommand = new QueuedCommand();
            queuedCommand.command = command;
            queuedCommand.context = context;
            queuedCommand.args = args;
            Instance.m_commandQueue.Enqueue(queuedCommand);
        }

        /* Execute a command */
        public static void Run(RequestContext context, string str)
        {
            if (str.Length > 0)
            {
                LogCommand(str);
                Instance.RecordCommand(str);
                Instance.m_commands.Run(context, str);
            }
        }
                ...
                foreach (Type type in assembly.GetTypes())
                {
                    ...
                    foreach (MethodInfo method in type.GetMethods(BindingFlags.Public | BindingFlags.Static))
                    {
                        ...
                        if (cb == null)
                        {
                            ...
                            if (cbs != null)
                            {
                                cb = delegate (RequestContext context, string[] args)
                                {
                                    cbs();
                                };
                            }
    ...
    class CommandTree : IEnumerable<CommandAttribute>
    {
        ...
        public void Run(RequestContext context, string commandStr)
        {
            ...
            _run(context, tokens, 0);
        }

        static string[] emptyArgs = new string[0] { };
        private void _run(RequestContext context, string[] commands, int index)
        {
            if (commands.Length == index)
            {
                RunCommand(context, emptyArgs);
                return;
            }
            ...
            if (!m_subcommands.ContainsKey(token))
            {
                RunCommand(context, commands.Skip(index).ToArray());
                return;
            }
            m_subcommands[token]._run(context, commands, index + 1);
        }

        private void RunCommand(RequestContext context, string[] args)
        {
            if (m_command == null)
            {
            ...
            {
                Console.Queue(m_command, context, args);
            }
            else
            {
                m_command.m_callback(context, args);
            }
        }
    }
}
